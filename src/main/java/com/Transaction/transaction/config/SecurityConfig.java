...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.cors().and().csrf().disable().authorizeRequests()
				/* ADMIN AUTHORIZATION */
				.antMatchers("/admin/post").hasAuthority("ADMIN_CREATE").antMatchers("/admin/updateBusStop/{id}")
				.hasAuthority("ADMIN_UPDATE").antMatchers("/admin/deleteBusStop/{id}").hasAuthority("ADMIN_DELETE")
				.antMatchers("/admin/postSeat/{id}").hasAuthority("ADMIN_CREATE").antMatchers("/admin/updateSeat/{id}")
				.hasAuthority("ADMIN_UPDATE").antMatchers("/admin/deleteSeat/{id}").hasAuthority("ADMIN_DELETE")
				...
				.antMatchers("/admin/routeBus/{id}").hasAuthority("ADMIN_CREATE").antMatchers("/admin/deleteBus/{id}")
				.hasAuthority("ADMIN_DELETE").antMatchers("/auth/create_user").permitAll().antMatchers("/auth/login")
				.permitAll()

				/* USER CAN ACCESS EVERY ENDPOINT */
				.antMatchers("/booking/**", "/payment/**", "/bookSeats/**", "/secret/**", "/tickets/**", "/user/**",
						"/busStop/**", "/route/**", "/bus/**", "/seat/**")
				.permitAll().anyRequest().authenticated().and().authenticationProvider(daoAuthenticationProvider())
