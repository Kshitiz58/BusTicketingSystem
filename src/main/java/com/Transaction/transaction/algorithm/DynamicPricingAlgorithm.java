...
public class DynamicPricingAlgorithm {
		...
		if (totalSeats != 0) {
			...
			if (availableSeats <= 10) {
				...
				dynamicPrice = dynamicPrice
						.multiply(HIGH_DEMAND_FACTOR.multiply(calculateDemandFactor(availableSeats, totalSeats)));
			} else {
				dynamicPrice = dynamicPrice
						.multiply(LOW_DEMAND_FACTOR.multiply(calculateDemandFactor(availableSeats, totalSeats)));
			}
		} else {
			return dynamicPrice;
		}
	...
	private BigDecimal calculateDemandFactor(int availableSeats, int totalSeats) {
		...
		return BigDecimal.valueOf(1.0)
				.add(BigDecimal.valueOf(1.0).subtract(BigDecimal.valueOf((double) availableSeats / totalSeats)));
	}
